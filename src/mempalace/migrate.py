...
def migrate(palace_path: str, dry_run: bool = False):  # noqa: PLR0915, C901
    ...
    palace_path = str(pathlib.Path(palace_path).expanduser())
    db_path = str(pathlib.Path(palace_path) / "chroma.sqlite3")
    ...
    print(f"  DB size:   {pathlib.Path(db_path).stat().st_size / 1024 / 1024:.1f} MB")
