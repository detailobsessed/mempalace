...
Corrects:
  ...
  - Common fat-finger words (3am → 3am, know → know)

Usage:
    from mempalace.spellcheck import spellcheck_user_text
    corrected = spellcheck_user_text("lsresdy know the question before")
...
def _get_speller():
    ...
    if _autocorrect_available is None:
        try:
            from autocorrect import Speller  # type: ignore[unresolved-import]  # ty: ignore[unresolved-import]
...
def _get_system_words() -> set:
    ...
    global _system_words  # noqa: PLW0603
    if _system_words is None:
        if _SYSTEM_DICT.exists():
            with Path(_SYSTEM_DICT).open(encoding="utf-8") as f:
...
def _should_skip(token: str, known_names: set) -> bool:  # noqa: PLR0911
...
def _load_known_names() -> set:
    ...
    try:
        ...
        for entity in reg._data.get("entities", {}).values():
            ...
            names.update(alias.lower() for alias in entity.get("aliases", []))
...
if __name__ == "__main__":
    test_cases = [
        "lsresdy know the question before",
        ...
        "Mempalace cant be fine-tuned if you alredy kno the question",
