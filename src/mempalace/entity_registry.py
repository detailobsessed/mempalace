...
def _wikipedia_lookup(word: str) -> dict:  # noqa: PLR0911
    """
    ...
    try:
        ...
        if any(phrase in extract for phrase in NAME_INDICATOR_PHRASES):
            ...
            confidence = 0.90 if any(f"{word.lower()} is a" in extract or f"{word.lower()} (name" in extract for _ in [1]) else 0.80
            return {
                "inferred_type": "person",
...
class EntityRegistry:
    ...
    def seed(self, mode: str, people: list, projects: list, aliases: dict | None = None):
        ...
        ambiguous = [name.lower() for name in self._data["people"] if name.lower() in COMMON_ENGLISH_WORDS]
    ...
    def lookup(self, word: str, context: str = "") -> dict:
        ...
        for canonical, info in self.people.items():
            if word.lower() == canonical.lower() or word.lower() in [a.lower() for a in info.get("aliases", [])]:
    ...
    @staticmethod
    def _disambiguate(word: str, context: str, person_info: dict) -> dict | None:
        """
