...
def _build_patterns(name: str) -> dict:
    ...
    n = re.escape(name)
    return {
        "dialogue": [re.compile(p.format(name=n), re.MULTILINE | re.IGNORECASE) for p in DIALOGUE_PATTERNS],
        "person_verbs": [re.compile(p.format(name=n), re.IGNORECASE) for p in PERSON_VERB_PATTERNS],
        "project_verbs": [re.compile(p.format(name=n), re.IGNORECASE) for p in PROJECT_VERB_PATTERNS],
...
def score_entity(name: str, text: str, lines: list) -> dict:  # noqa: C901, PLR0912
    """
...
def detect_entities(file_paths: list, max_files: int = 10) -> dict:
    ...
    for name, frequency in sorted(candidates.items(), key=operator.itemgetter(1), reverse=True):
    ...
    people.sort(key=operator.itemgetter("confidence"), reverse=True)
    projects.sort(key=operator.itemgetter("confidence"), reverse=True)
    uncertain.sort(key=operator.itemgetter("frequency"), reverse=True)
...
def confirm_entities(detected: dict, yes: bool = False) -> dict:  # noqa: C901, PLR0912, PLR0915
    """
    ...
    if choice == "edit":
        ...
        remove = input("  Numbers to REMOVE from people (comma-separated, or enter to skip): ").strip()
        if remove:
        ...
        remove = input("  Numbers to REMOVE from projects (comma-separated, or enter to skip): ").strip()
        if remove:
            to_remove = {int(x.strip()) - 1 for x in remove.split(",") if x.strip().isdigit()}
    ...
    if choice == "add" or input("\n  Add any missing? [y/N]: ").strip().lower() == "y":
        while True:
            ...
            if not name:
                break
            kind = input(f"  Is '{name}' a (p)erson or p(r)object? ").strip().lower()
            if kind == "p":
