...
class GitignoreMatcher:
    ...
    @classmethod
    def from_dir(cls, dir_path: Path):  # noqa: C901
    ...
    @staticmethod
    def _match_from_root(target_parts: list, pattern_parts: list) -> bool:
...
def is_force_included(path: Path, project_path: Path, include_paths: set) -> bool:  # noqa: PLR0911
...
def chunk_text(content: str, _source_file: str) -> list:
    """
...
def get_collection(palace_path: str):
    Path(palace_path).mkdir(exist_ok=True, parents=True)
...
def add_drawer(  # noqa: PLR0913, PLR0917
    collection,
    wing: str,
    room: str,
    content: str,
...
def process_file(  # noqa: PLR0913, PLR0917
    filepath: Path,
    project_path: Path,
    collection,
    wing: str,
...
def scan_project(
    ...
    for root, dirs, filenames in os.walk(project_path):
        ...
        if respect_gitignore:
            active_matchers = [
                matcher for matcher in active_matchers if root_path == matcher.base_dir or matcher.base_dir in root_path.parents
            ]
        ...
        dirs[:] = [d for d in dirs if is_force_included(root_path / d, project_path, include_paths) or not should_skip_dir(d)]
        ...
        for filename in filenames:
            ...
            if filepath.suffix.lower() not in READABLE_EXTENSIONS and not exact_force_include:
                continue
            if respect_gitignore and active_matchers and not force_include and is_gitignored(filepath, active_matchers, is_dir=False):
                continue
            files.append(filepath)
    return files


def mine(  # noqa: PLR0913, PLR0917
    project_dir: str,
    palace_path: str,
    ...
    print("\n  By room:")
    for room, count in sorted(room_counts.items(), key=operator.itemgetter(1), reverse=True):
...
def status(palace_path: str):
    ...
    for wing, rooms in sorted(wing_rooms.items()):
        print(f"  WING: {wing}")
        for room, count in sorted(rooms.items(), key=operator.itemgetter(1), reverse=True):
