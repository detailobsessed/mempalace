...
class Dialect:
    ...
    @staticmethod
    def encode_emotions(emotions: list[str]) -> str:
    ...
    def _detect_entities_in_text(self, text: str) -> list[str]:
        ...
        for i, w in enumerate(words):
            ...
            if len(clean) >= 2 and clean[0].isupper() and clean[1:].islower() and i > 0 and clean.lower() not in _STOP_WORDS:
                code = clean[:3].upper()
    ...
    def compress(self, text: str, metadata: dict | None = None) -> str:  # noqa: PLR0914
        """
    ...
    @staticmethod
    def extract_key_quote(zettel: dict) -> str:  # noqa: C901
        ...
        if quotes:
            ...
            unique = []
            for raw_q in quotes:
                stripped_q = raw_q.strip()
                if stripped_q not in seen and len(stripped_q) >= 8:
                    seen.add(stripped_q)
                    unique.append(stripped_q)
            quotes = unique
    ...
    @staticmethod
    def encode_tunnel(tunnel: dict) -> str:
    ...
    def encode_file(self, zettel_json: dict) -> str:
        ...
        lines.extend(self.encode_zettel(z) for z in zettel_json.get("zettels", []))

        lines.extend(self.encode_tunnel(t) for t in zettel_json.get("tunnels", []))
    ...
    def compress_file(self, zettel_json_path: str, output_path: str | None = None) -> str:
        ...
        with Path(zettel_json_path).open(encoding="utf-8") as f:
            data = json.load(f)
        dialect = self.encode_file(data)
        if output_path:
            Path(output_path).write_text(dialect, encoding="utf-8")
        return dialect
    ...
    def compress_all(self, zettel_dir: str, output_path: str | None = None) -> str:
        ...
        all_dialect = []
        zettel_path = Path(zettel_dir)
        for fpath in sorted(zettel_path.iterdir()):
            if fpath.name.endswith(".json"):
                with fpath.open(encoding="utf-8") as f:
                    data = json.load(f)
                dialect = self.encode_file(data)
                all_dialect.extend((dialect, "---"))
        combined = "\n".join(all_dialect)
        if output_path:
    ...
    def generate_layer1(  # noqa: C901, PLR0912, PLR0914, PLR0915
        self,
        zettel_dir: str,
        ...
        for fpath in sorted(zettel_path.iterdir()):
            ...
            file_num = fpath.stem.replace("file_", "")
            ...
            for z in data.get("zettels", []):
                ...
                has_key_flag = any(f in flags for f in ["ORIGIN", "CORE", "GENESIS"]) if flags else False
        ...
        for fpath in sorted(zettel_path.iterdir()):
            ...
            with fpath.open(encoding="utf-8") as f:
                data = json.load(f)
            all_tunnels.extend(data.get("tunnels", []))
        ...
        lines = []
        lines.extend(("## LAYER 1 -- ESSENTIAL STORY", f"## Auto-generated from zettel files. Updated {datetime.now(tz=UTC).date()}.", ""))
        ...
        for date_key in sorted(by_date.keys()):
            ...
            for z, _fnum in by_date[date_key]:
                ...
                parts = [ent_str]
                title = z.get("title", "")
                hint = title.split(" - ", 1)[1][:30] if " - " in title else "_".join(z.get("topics", [])[:2])
                if hint:
                    parts.append(hint)
    ...
    @staticmethod
    def decode(dialect_text: str) -> dict:
...
if __name__ == "__main__":
    ...
    if args[0] == "--init":
        ...
        print(f"Original: ~{stats['original_tokens_est']} tokens est ({stats['original_chars']} chars)")
        print(f"AAAK:     ~{stats['summary_tokens_est']} tokens est ({stats['summary_chars']} chars)")
