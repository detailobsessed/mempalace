...
def _load_known_names_config(force_reload: bool = False):
    ...
    global _KNOWN_NAMES_CACHE  # noqa: PLW0603
...
def extract_subject(lines):
    ...
    """
    skip_patterns = re.compile(r"^(\.\/|cd |ls |python|bash|git |cat |source |export |claude|./activate)")
    for line in lines:
        if line.startswith("> "):
            prompt = line[2:].strip()
...
def split_file(filepath, output_dir, dry_run=False):
    ...
    for i, (start, end) in enumerate(itertools.pairwise(boundaries)):
        ...
        ts_human, _ts_iso = extract_timestamp(chunk)
...
def main():
    parser = argparse.ArgumentParser(description="Split concatenated transcript mega-files into per-session files")
    parser.add_argument(
        "--source",
        type=str,
        default=None,
