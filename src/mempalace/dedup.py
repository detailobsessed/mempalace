...
def get_source_groups(col, min_count: int = MIN_DRAWERS_TO_CHECK, source_pattern: str | None = None) -> dict[str, list[str]]:
    ...
    while offset < total:
        ...
        if not batch["ids"]:
            break
        for did, meta in zip(batch["ids"], batch["metadatas"], strict=True):
...
def dedup_source_group(
    ...
    *,
    ...
    items = list(zip(data["ids"], data["documents"], data["metadatas"], strict=True))
    ...
    for did, doc, _meta in items:
        ...
        try:
            ...
            is_dup = False
            for rid, dist in zip(results["ids"][0], dists, strict=True):
...
def show_stats(palace_path: str | None = None) -> None:
...
def dedup_palace(
    ...
    *,
    ...
    min_count: int = MIN_DRAWERS_TO_CHECK,
) -> None:
    ...
    try:
        ...
        for i, (src, drawer_ids) in enumerate(sorted_groups):
            kept, deleted = dedup_source_group(col, drawer_ids, threshold, dry_run=dry_run)
            total_kept += len(kept)
            total_deleted += len(deleted)

            if deleted:
                print(f"  [{i + 1:3d}/{len(groups)}] {src[:50]:50s} {len(drawer_ids):4d} → {len(kept):4d}  (-{len(deleted)})")

        elapsed = time.time() - t0

        print(f"\n{'─' * 55}")
        print(f"  Done in {elapsed:.1f}s")
        print(f"  Drawers: {total_kept + total_deleted:,} → {total_kept:,}  (-{total_deleted:,} removed)")
