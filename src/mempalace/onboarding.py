...
def _auto_detect(directory: str, known_people: list) -> list:
    ...
    try:
        ...
        return [e for e in detected["people"] if e["name"].lower() not in known_names and e["confidence"] >= 0.7]
    except Exception:
        return []
...
def _warn_ambiguous(people: list) -> list:
    ...
    return [p["name"] for p in people if p["name"].lower() in COMMON_ENGLISH_WORDS]
...
def _generate_aaak_bootstrap(people: list, projects: list, wings: list, mode: str, config_dir: Path | None = None):  # noqa: C901
    """
    ...
    if projects:
        ...
        facts_lines.extend(f"- **{proj}**" for proj in projects)
        facts_lines.append("")
...
def run_onboarding(
    ...
    if auto_detect and _yn("\nScan your files for additional names we might have missed?"):
        ...
        if detected:
            ...
            for e in detected:
                print(f"    {e['name']:20} confidence={e['confidence']:.0%}  ({', '.join(e['signals'][:1])})")
            print()
